...
class Yxy extends Color
{
     ...
     *
     * @param float|string|array $Y – Y
     * or string or array representation
     * @param float $x The x
     * @param float $y The y
     */
    public function __construct(float|string|array $Y, float $x = null, float $y = null)
    {
        $this->toSelf = "toYxy";

        if( !is_numeric($Y) && is_string($Y) && strpos($x, ',') === false  ){
            // Probalby color name
            if( $hex = $this->getColorFromName($Y) ){
                [$Y, $x, $y] = $hex->toYxy()->toArray();
                
            } else {
                throw new InvalidArgumentException('Color name "' . $Y . '" does not exist');
            }
        }
