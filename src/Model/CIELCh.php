...
class CIELCh extends Color
{
     ...
     * @param float $hue - hue
     */
    public function __construct(float|string|array $lightness, float $chroma = null, float $hue = null)
    {
        $this->toSelf = "toCIELCh";

        if( !is_numeric($lightness) && is_string($lightness) && strpos($lightness, ',') === false  ){
            // Probalby color name
            if( $hex = $this->getColorFromName($lightness) ){
                [$lightness, $chroma, $hue] = $hex->toCIELCh()->toArray();
            } else {
                throw new InvalidArgumentException('Color name "' . $lightness . '" does not exist');
            }
        }
